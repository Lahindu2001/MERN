...
function ProductManager() {
  ...
  const [selectedFields, setSelectedFields] = useState({
    product_id: true, // Include product_id for display
    product_name: true,
    price: true,
    description: true,
    photo: true,
    ...
    <div className="product-section">
      ...
      <div className="users-table-container">
        ...
        <table className="users-table">
          <thead>
            <tr>
              <th>product_id</th> {/* Add product_id to table header */}
              ...
              <tr key={product._id}>
                {editingProductId === product._id ? (
                  <td colSpan={Object.keys(defaultInputs).length + 2}>
                    <div className="update-user-container">
                      ...
                      <form onSubmit={handleUpdateProduct}>
                        {["product_name", "price", "description", "photo", "created_by"].map(
                          (field) => (
                  ...
                  <>
                    ...
                    {Object.keys(defaultInputs).map((field) => (
                      <td key={field}>
                        {field === "photo" && product[field] ? (
                          <img
                            src={`http://localhost:5000${product[field]}`}
                            alt="product"
                            width="60"
                          />
                        ) : (
                    ...
                    <td>
                      <button className="update-button" onClick={() => startEdit(product)}>
                        Edit
                      </button>
                      <button
                        className="delete-button"
